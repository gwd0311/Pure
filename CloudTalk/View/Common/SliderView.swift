...
struct SliderPathBetweenView: View {
    ...
    var body: some View {
        ...
        .stroke(ColorManager.redLight, lineWidth: slider.lineWidth)
    }
}
...
class CustomSlider: ObservableObject {
    ...
    func initialize() {
        self.lowHandle.currentLocation = CGPoint(x: 0, y: self.lowHandle.sliderHeight / 2)
        self.highHandle.currentLocation = CGPoint(x: self.highHandle.sliderWidth, y: self.highHandle.sliderHeight / 2)
    }
    
    func setSlider(lowHandleValue: Int, highHandleValue: Int) {
        guard lowHandleValue >= Int(self.valueStart), highHandleValue <= Int(self.valueEnd), lowHandleValue > highHandleValue else { return }
        
        // 슬라이더 핸들의 Location에서 CGPoint값을 조정해서 각각 넣어야함
        self.lowHandle.currentLocation = CGPoint(x: self.lowHandle.sliderWidth, y: self.lowHandle.sliderHeight / 2)
        self.highHandle.currentLocation = CGPoint(x: self.highHandle.sliderWidth, y: self.highHandle.sliderHeight / 2)
    }
}
